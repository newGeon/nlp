...
# 명사 문장 만들기 (리스트 변환)
def fn_make_noun_sentences(model_komoran, list_sentence):
    result = []
...
if __name__=="__main__":
    ...
    for row in list_book:
        ...
        if book_introduction != '':
            ...
            list_noun_sentence_introduction = fn_make_noun_sentences(komoran, list_sentence_introduction)

            if len(list_noun_sentence_introduction) > 1:
                list_keyword_introduction = keyword_extractor_v1.summarize(list_noun_sentence_introduction, topk=100)
            # print('===================================================================')
        
        if book_content != '':
            list_sentence_content = fn_text_split_sentence(book_content)
            list_noun_sentence_content = fn_make_noun_sentences(komoran, list_sentence_content)

            if len(list_noun_sentence_content) > 1:
                list_keyword_content = keyword_extractor_v2.summarize(list_noun_sentence_content, topk=100)
            # print('===================================================================')
            
        if book_instroy != "['']" and book_instroy != '[]':
            list_sentence_instroy = fn_list_split_sentece(book_instroy, 'instroy')
            list_noun_sentence_instroy = fn_make_noun_sentences(komoran, list_sentence_instroy)
            
            if len(list_noun_sentence_instroy) > 1:
                list_keyword_instroy = keyword_extractor_v1.summarize(list_noun_sentence_instroy, topk=100)
            # print('===================================================================')

        if publisher_review != '':
            list_sentece_review = fn_text_split_sentence(publisher_review)
            list_noun_sentence_review = fn_make_noun_sentences(komoran, list_sentece_review)

            if len(list_noun_sentence_review) > 1:
                list_keyword_review = keyword_extractor_v1.summarize(list_noun_sentence_review, topk=100)
            # print('===================================================================')
        ...
        if book_recomendation != "['']" and book_recomendation != "[]":
            ...
            list_noun_sentence_recomendation = fn_make_noun_sentences(komoran, list_sentence_recomendation)
            
            if len(list_noun_sentence_recomendation) > 1:
                list_keyword_recomendation = keyword_extractor_v1.summarize(list_noun_sentence_recomendation, topk=100)
            # print('===================================================================')
