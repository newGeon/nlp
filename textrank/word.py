...
def word_graph(sents, tokenize=None, min_count=2, window=2,
    min_cooccurrence=2, vocab_to_idx=None, verbose=False, data_type='general'):
    """
    Arguments
    ---------
    sents : list of str
    ...
    if vocab_to_idx is None:
        ...
        idx_to_vocab, vocab_to_idx = scan_vocabulary(sents, min_count, data_type)
    else:
        idx_to_vocab = [vocab for vocab, _ in sorted(vocab_to_idx.items(), key=lambda x:x[1])]

    tokens = tokenize_sents(sents, tokenize, data_type)
